...
namespace XMSTaxonomyManagment.Controllers
{
    public class LocalReportController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            LocalReportMainViewModel ViewModel;

            using (var repository = GetRepository())
            {
                taxonomiesList = repository.TaxonomyRepository.GetAllTaxonomies();
                var taxonomy = taxonomiesList.FirstOrDefault();
                sourceIds = repository.TaxonomyReportRepository.GetShortTaxonomyReportsByTaxonomyId(taxonomy.TaxonomyId);

                var taxonomySelectmodel = new List<TaxonomyDescriptionViewModel>();

                foreach (var entity in taxonomiesList)
                {
                    taxonomySelectmodel.Add(new TaxonomyDescriptionViewModel()
                    {
                ...
                SourceIdViewModel sourceIdModel;

                if (sourceId == null)
                {
                    sourceIdModel = new SourceIdViewModel()
                    {
                        Description = "",
                        Id = null
                    };
                ...
                ViewModel = new LocalReportMainViewModel()
                {
                    Taxonomies = taxonomySelectmodel.ToArray(),
                    SelectedTaxonomy = taxonomySelectmodel.FirstOrDefault()
                };
            }
